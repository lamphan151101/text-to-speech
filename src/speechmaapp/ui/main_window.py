...
from speechmaapp import __version__
from speechmaapp.config import AppConfig, Settings
from speechmaapp.core.speechma_engine import fetch_captcha_image, submit_captcha
from speechmaapp.core.subtitle_parser import parse_input_file
from speechmaapp.core.voices_catalog import get_grouped_voices
from speechmaapp.models.speaker import Speaker
from speechmaapp.models.subtitle import Segment
from speechmaapp.ui.animated_progress import AnimatedProgressBar
from speechmaapp.ui.captcha_dialog import CaptchaDialog
from speechmaapp.ui.output_panel import OutputPanel
from speechmaapp.ui.speaker_manager import SpeakerManager
from speechmaapp.ui.subtitle_table import SubtitleTable
from speechmaapp.ui.theme import build_stylesheet
from speechmaapp.utils.i18n import LANG_EN, LANG_VI, tr, translator
from speechmaapp.utils.logging_utils import log_error, log_info
from speechmaapp.workers.preview_worker import PreviewWorker
from speechmaapp.workers.tts_worker import TtsWorker
...
class MainWindow(QMainWindow):
    def __init__(self, config: AppConfig) -> None:
        ...
        self.voice_groups = get_grouped_voices(self.config.voices_path)
        ...
        # Keeps Python references to running PreviewWorkers so Qt cannot GC them mid-thread.
        ...
        self.captcha_button = QPushButton(tr("top.captcha"))
        self.captcha_button.clicked.connect(self._on_captcha_btn)
        self.captcha_button.setMinimumWidth(80)

        self.language_button = QPushButton()
        self.language_button.setMinimumWidth(60)
        ...
        top_bar_layout.addWidget(self.captcha_button)
    ...
    def _seed_auto_speakers(self) -> None:
        ...
        for name, values in temp.items():
            self.speaker_manager.speakers[name] = Speaker(name=name, segments=values)
        if temp:
    ...
    def _build_segment_pitch_map(self) -> dict[int, int]:
        pitch_map: dict[int, int] = {}
        for speaker in self.speaker_manager.speakers.values():
            for seg_index in speaker.segments:
                pitch_map[seg_index] = speaker.pitch
        return pitch_map

    def _build_segment_rate_map(self) -> dict[int, int]:
        rate_map: dict[int, int] = {}
        for speaker in self.speaker_manager.speakers.values():
            for seg_index in speaker.segments:
                rate_map[seg_index] = speaker.rate
        return rate_map
    ...
    def preview_selected(self) -> None:
        ...
        if not voice:
            ...
            return
        pitch = self._build_segment_pitch_map().get(seg.index, 0)
        rate = self._build_segment_rate_map().get(seg.index, 0)
        log_info(f"Preview selected segment={seg.index} voice={voice} pitch={pitch} rate={rate}")
        self._launch_preview(seg.text[:300], voice, pitch, rate)

    def _preview_by_row(self, row: int, _column: int) -> None:
        self.subtitle_table.selectRow(row)
        self.preview_selected()

    def preview_custom_voice(self, text: str, voice: str, pitch: int, rate: int) -> None:
        log_info(f"Preview custom voice={voice} pitch={pitch} rate={rate}")
        self._launch_preview(text, voice, pitch, rate)

    def _launch_preview(self, text: str, voice: str, pitch: int, rate: int) -> None:
        """Start a PreviewWorker, keeping a reference alive until the thread finishes.

        Without this list, Python GC-s the old worker while its QThread is still running,
        which causes the fatal 'QThread: Destroyed while thread is still running' crash.
        """
        self._set_preview_enabled(False)
        worker = PreviewWorker(self.config, text, voice, pitch=pitch, rate=rate)
        worker.finished.connect(self._play_audio_file)
        worker.error.connect(self._on_preview_error)
        # Remove from keeper list when done, then re-enable buttons if no more workers.
        worker.finished.connect(lambda _path, w=worker: self._cleanup_preview_worker(w))
        worker.error.connect(lambda _msg, w=worker: self._cleanup_preview_worker(w))
        self._preview_workers.append(worker)
        self.preview_worker = worker  # kept for backward-compat reads
        worker.start()

    def _cleanup_preview_worker(self, worker: PreviewWorker) -> None:
        try:
    ...
    def _set_preview_enabled(self, enabled: bool) -> None:
        self.subtitle_preview_button.setEnabled(enabled)
        self.speaker_manager.preview_btn.setEnabled(enabled)
    ...
    def export_audio(self) -> None:
        ...
        if missing:
            ...
            QMessageBox.warning(
                self,
                tr("dlg.missing_voice_title"),
                tr("dlg.missing_voice_msg", items=", ".join(map(str, missing))),
            )
            return
        ...
        self.worker = TtsWorker(
            ...
            segment_pitch_map=self._build_segment_pitch_map(),
            segment_rate_map=self._build_segment_rate_map(),
    ...
    def _on_captcha_btn(self) -> None:
        try:
            image = fetch_captcha_image()
        except Exception as exc:
            log_error(f"Fetch captcha failed: {exc}")
            QMessageBox.critical(self, tr("dlg.error_title"), tr("captcha.network_error"))
            return
        dialog = CaptchaDialog(self, initial_image=image)
        while True:
            ...
            if submit_captcha(code):
                log_info("Captcha verified successfully")
                return
            dialog.show_wrong_code_error()

    # ------------------------------------------------------------------
    # Preview error / player
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Tooltip / status / retranslate
    # ------------------------------------------------------------------

    def _apply_tooltips(self) -> None:
        self.output_panel.export_button.setToolTip(tr("tip.export"))
        self.subtitle_preview_button.setToolTip(tr("tip.preview_row"))
        self.speaker_manager.pitch_spinbox.setToolTip(tr("speaker.pitch_tip"))
        self.speaker_manager.rate_spinbox.setToolTip(tr("speaker.rate_tip"))
    ...
    def _update_header_status(self) -> None:
        ...
        self.header_status.setText(
            tr("status.file_info", file=file_name, mode=self._current_mode_text(), count=len(self.segments))
        )

    def _update_ui_state(self) -> None:
        has_segments = bool(self.segments)
        has_mapping = (
            all(bool(self._build_segment_voice_map().get(seg.index, "")) for seg in self.segments)
            if has_segments
            else False
        )
    ...
    def retranslate_ui(self) -> None:
        ...
        self.captcha_button.setText(tr("top.captcha"))
        self.help_button.setText(tr("top.help"))
        self.language_button.setText(
            tr("top.lang_toggle_to_en") if translator.language == LANG_VI else tr("top.lang_toggle_to_vi")
        )
        self.play_button.setText(tr("player.play"))
